 ...
 *    「◔ 5h:xx%」小药丸，圆环进度条按剩余量绿/黄/红着色（画法复刻 dsh 上下文用量环）；
 *    悬停展开详情小悬窗（含两个窗口完整信息与刷新按钮）、移走即关，
 *    悬停中点击固定展开、再次点击关闭。
 *
...
window.__ModuleLoader__.load({
    ...
    factory: function (require) {
        ...
        // remote-web-ui 门控通道镜像前缀：手机侧直连会被 host 路由的回环限制 403，
        // 改走 /remote 通道（凭据验证后由通道在回环重新发起）。
        var REMOTE_API_PATH = '/remote' + API_PATH;

        // remote-web-ui 的 compact 模式把 composer 的 trailing 行塌缩为零宽，
        // 徽章会溢出到发送按钮右侧；此时把徽章锚到行右缘（发送按钮与上下文圆环左侧）。
        var MOBILE_FIX_CSS = 'body.dsh-remote-compact-picker [data-dsh-kimi-quota-badge]'
            + '{position:absolute !important;right:112px;top:50%;transform:translateY(-50%);z-index:3}';
        var MOBILE_FIX_STYLE_ID = 'dsh-kimi-quota/mobile-fix.css';

        function fetchQuotaJson() {
            return fetch(API_PATH, { headers: { accept: 'application/json' } })
                .then(function (r) {
                    if (r.status !== 403) return r;
                    var device = null;
                    try { device = window.sessionStorage.getItem('dsh-remote-device'); } catch (e) { /* 无 sessionStorage */ }
                    if (!device) return r;
                    return fetch(REMOTE_API_PATH, {
                        headers: { accept: 'application/json', 'x-dsh-remote-device': device },
                    });
                });
        }
        ...
        function parseUsages(raw) {
            ...
            // API 未下发任何周窗口字段（新会员无周限制）：标记 unlimited，界面灰显「无周限制」。
            if (!hasWeekly) result.weekly = { pct: null, remaining: null, limit: null, resetMs: 0, unlimited: true };
            return result;
        }
        ...
        var S = {
            ...
            barTrack: { height: 6, borderRadius: 999, background: 'rgba(128,128,128,0.2)', overflow: 'hidden' },
            ...
            popover: {
                ...
                // 弹窗整体 scale(0.75)，border-radius 会随缩放缩小；
                // 除以 0.75 补偿，使视觉圆角与原生弹层（MenuSurface lg=16px）一致。
                borderRadius: 'calc(var(--dsw-radius-lg, 16px) / 0.75)',
                boxShadow: 'var(--dsw-elevation-prominent, 0 8px 28px rgba(0,0,0,0.35))',
                padding: '12px 14px',
                display: 'flex',
                flexDirection: 'column',
                gap: 12,
                cursor: 'default',
                // 整体缩小到 3/4，右下为原点保持与徽章贴合，同时避免小屏左溢出。
        ...
        // 圆环进度条——1:1 复刻 dsh 对话框下方上下文用量环的画法
        // （dsh-client-ui-conversation ContextMeter：14×14 SVG、r=5.5、
        // stroke-width 2、圆头、stroke-dasharray 弧长、-90° 起笔）。
        var RING_R = 5.5;
        var RING_C = 2 * Math.PI * RING_R;
        function quotaRing(pct, color) {
            ...
            return h('svg', {
                ...
                h('circle', {
                    cx: 7, cy: 7, r: RING_R,
                    fill: 'none', stroke: 'rgba(128,128,128,0.35)', strokeWidth: 2,
                }),
                h('circle', {
                    cx: 7, cy: 7, r: RING_R,
                    fill: 'none', stroke: color, strokeWidth: 2, strokeLinecap: 'round',
                    strokeDasharray: (RING_C * p / 100) + ' ' + RING_C,
        ...
        function QuotaRow(props) {
            ...
            return h('div', { style: S.row },
                h('div', { style: S.rowHead },
                    h('span', { style: S.rowLabel }, props.label),
                    h('span', { style: S.rowValue }, '剩余 ',
                        h('span', { style: { color: colorFor(q.pct), fontSize: 16, fontWeight: 600 } }, pctText))),
        ...
        // 输入框工具行紧凑徽章：「● 5h:xx%」。
        // 悬停：浅灰待选遮罩 + 自动展开详情悬窗，移走即关；
        // 悬停中点击：固定展开（移走不关），再次点击关闭。
        function KimiQuotaBadge() {
            ...
            var pinned = _p[0]; var setPinned = _p[1];
            var _hov = useState(false);
            var hover = _hov[0]; var setHover = _hov[1];
            ...
            var badgeStyle = (hover || pinned)
                ? Object.assign({}, S.badge, { background: 'rgba(128,128,128,0.18)', opacity: 1 })
                : S.badge;

            return h('div', {
                ref: wrapRef,
                style: S.badgeWrap,
                'data-dsh-kimi-quota-badge': '',
                onMouseEnter: function () { setHover(true); setOpen(true); },
                onMouseLeave: function () { setHover(false); if (!pinned) setOpen(false); },
            },
                h('button', {
                    type: 'button',
                    style: badgeStyle,
                    title: 'Kimi Code 额度（悬停查看详情，点击固定展开）',
                    onClick: function () {
                        if (pinned) { setPinned(false); setOpen(false); }
                        else { setPinned(true); setOpen(true); }
                    },
                },
                    h('span', { style: { display: 'inline-flex', flexShrink: 0 } }, quotaRing(state.phase === 'error' ? 100 : pct, dotColor)),
                    h('span', null, label)),
        ...
        function apply(ctx) {
            ...
            // 注入 compact 模式位置修复样式（幂等，只针对 remote-web-ui 的 compact 类名）
            if (typeof document !== 'undefined' && document.getElementById(MOBILE_FIX_STYLE_ID) === null) {
                var styleEl = document.createElement('style');
                styleEl.id = MOBILE_FIX_STYLE_ID;
                styleEl.textContent = MOBILE_FIX_CSS;
